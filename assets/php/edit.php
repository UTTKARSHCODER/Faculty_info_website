<?php
    ...
    if($_SERVER['REQUEST_METHOD'] === 'POST'){
        if(isset($_GET['edit_id1'])) {
            ...
            $query = "UPDATE `user_details` SET `username`= '$new_email' WHERE `user_details`.`username`='$email'";
        ...
        // $data = mysqli_fetch_assoc($result);
            ...
            $output = '';
            $email = $_POST['edit_id'];
            $query = "SELECT `username` FROM `user_details` WHERE `user_details`.`username` = '$email'";
            $result1 = mysqli_query($conn, $query);
            if (mysqli_num_rows($result1) > 0) {
                $row = mysqli_fetch_assoc($result1); 
                $output .= '
                <form action = "assets/php/edit.php?edit_id1='. $row["username"] .'"  method="POST">
                    <div style = "display: flex;">
                        <div class = "mb-3">
                            <label for = "email" class = "form-label">Email</label>
                            <input type = "text" class = "form-control" id = "updated_email" value = "'.$row["username"].'" placeholder="Enter email"  name = "updated_email">
                        </div>
